...
# Capture both stdout and stderr, allow command to "fail" with verbose output
set +e
...
# Extract PR URL from output (gh pr create prints URL as last line, but may have verbose output)
PR_URL=$(echo "$PR_OUTPUT" | grep -oE 'https://github.com/[^/]+/[^/]+/pull/[0-9]+' | head -1)

if [ -z "$PR_URL" ]; then
    echo "❌ Failed to create PR or extract PR URL"
    echo ""
    echo "Debug output:"
    echo "$PR_OUTPUT"
    echo ""
