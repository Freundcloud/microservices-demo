...
if [ -n "$EXISTING_PR" ]; then
    echo "ℹ️  PR already exists for branch $BRANCH: $EXISTING_PR"
    PR_URL="$EXISTING_PR"
else
    # Create new PR - capture both stdout (URL) and stderr (errors)
    # Use a temporary file to capture stderr while getting stdout
...
if [ -z "$PR_URL" ] || [[ ! "$PR_URL" =~ ^https://github.com/ ]]; then
    echo "❌ Failed to get valid PR URL: '$PR_URL'"
    echo ""
    echo "You can create the PR manually:"
    echo "  gh pr create --base main --head $BRANCH --title 'Release v${VERSION}' --fill"
    exit 1
