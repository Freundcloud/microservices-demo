...
# Install Istio Base
resource "null_resource" "install_istio_base" {
...
resource "null_resource" "install_istiod" {
  count = var.enable_istio ? 1 : 0
  ...
  provisioner "local-exec" {
    command = <<-EOT
      ...
      # Install or upgrade istiod
      helm upgrade --install istiod istio/istiod \
        --namespace istio-system \
        --version ${var.istio_version} \
        --set global.hub=docker.io/istio \
        --set global.tag=${var.istio_version} \
  ...
  depends_on = [null_resource.install_istio_base]
}

# Install Istio Ingress Gateway
resource "null_resource" "install_istio_ingressgateway" {
  count = var.enable_istio ? 1 : 0
  ...
  depends_on = [null_resource.install_istiod]
}
